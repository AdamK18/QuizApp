import React, {useState} from 'react'
import { connect } from 'react-redux'
import {createArrayN, shuffle} from '../shared/Utils'
import './main.css'

export const Main = (props) => {
    const {round, user} = props
    let [score, setScore] = useState(0);
    let [correct] = useState(false);
    let [current, setCurrent] = useState(0)
    let [canSkip] = useState(false)

    const check = (i) => {
        if(currentQuestion.correct == i){
            correct = true
        }
        else{
            correct = false
        }
        console.log(currentQuestion.correct + " " + i)
    }

    const checkAnswer = () => {
        if(correct){
            correct = false
            canSkip = true
        }
    }
    const nextQuestion = () => {
        if(canSkip) {
            setCurrent(current + 1)
            setScore(score+1)
            canSkip = false
        }

    }

    const checkstates = () => {
        console.log(correct)
        console.log(score)
        console.log(canSkip)
        
    }
     
    const questions = shuffle(createArrayN(round.list.length))
    let currentQuestion = round.list[questions[current]];

    return (
        <div className="main">
            <div className="main-details">
                <h1>Hello {user.name}</h1>
                <p>Score: {score}</p>
            </div>
            <p>{currentQuestion.question}</p>
            <div className="main-game">
                {currentQuestion.answers.map((answers,i) =>
                    <React.Fragment>
                        <button onClick={() => check(i+1)}>{answers}</button>
                    </React.Fragment>
                )}
            </div>
            <div className="main-utilities">
                <button onClick={checkAnswer}>Check</button>
                <button onClick={nextQuestion}>Next</button>
                <button onClick={checkstates}>states</button>
            </div>
        </div>
    )
}

const mapStateToProps = (state) => {
    return{
        round: state.round,
        user: state.user
    }
}

export default connect(mapStateToProps, null)(Main)

